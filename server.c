...
const char *get_file_type(const char *file_ext) {
...
void build_http_response(const char *file_name, const char *file_ext, char *response, size_t *response_len) {
    // build HTTP header
    const char *file_type = get_file_type(file_ext);
    ...
    // if file not exist and filename is not set, serve index.html
    int file_fd = open(file_name, O_RDONLY);
    if (file_name && file_fd == -1) {
